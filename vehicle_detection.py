...
size_thresholds = {
    'Class 1': (60, 5),    # Motorcars, Taxis & Small MPVs (width between 60-100px for example)
    'Class 2': (100, 120), # Small Vans, Big MPVs & Utilities
    'Class 3': (120, 180), # Lorries & Large Vans (2 axles)
    'Class 4': (180, 1000),# Lorries with 3 axles & Construction vehicles (very large width)
    'Class 5': (120, 300), # Bus (height could also be checked, usually tall & long)
    'Class 6': (0, 60),    # Motorcycles & Scooters (small width)
}

def map_to_six_classes(yolo_cls, w, h):
    """
    Maps YOLOv8 COCO class and bounding box size to one of the six vehicle classes.
    Arguments:
      - yolo_cls (int): YOLO COCO class id (car=2, motorcycle=3, bus=5, truck=7)
      - w (int): bounding box width
      - h (int): bounding box height
    Returns:
      - label (str): vehicle class as string, or None if not matched.
    """
    if yolo_cls == 2:  # car
        if w >= size_thresholds['Class 1'][0] and w < size_thresholds['Class 2'][0]:
            return 'Class 1'
        elif w >= size_thresholds['Class 2'][0] and w < size_thresholds['Class 3'][0]:
            return 'Class 2'
        elif w >= size_thresholds['Class 3'][0] and w < size_thresholds['Class 4'][0]:
            return 'Class 3'
        elif w >= size_thresholds['Class 4'][0]:
            return 'Class 4'
    elif yolo_cls == 7:  # truck
        if w >= size_thresholds['Class 3'][0] and w < size_thresholds['Class 4'][0]:
            return 'Class 3'
        elif w >= size_thresholds['Class 4'][0]:
            return 'Class 4'
    elif yolo_cls == 5:  # bus
        if w >= size_thresholds['Class 5'][0]:
            return 'Class 5'
    elif yolo_cls == 3:  # motorcycle
        if w < size_thresholds['Class 6'][1]:
            return 'Class 6'
    return None
...
USE_NVENC = False  # True if you have NVIDIA GPU + ffmpeg with h264_nvenc
try:
...
while running:
    ...
    if start_processing and polygon_defined and time_start:
        ...
        for box in res.boxes:
            ...
            label = map_to_six_classes(cls_id, w, h)
            if label is None:
                continue  # skip if not mapped

            # Optional: set color by class
            color_map = {
                "Class 1": (255,255,255),
                "Class 2": (0,255,0),
                "Class 3": (255,0,0),
        ...
        for objectID, trk_centroid in objects.items():
            ...
            if not already_counted:
                ...
                if (
                    ...
                    if (last_crop is not None) and filename:

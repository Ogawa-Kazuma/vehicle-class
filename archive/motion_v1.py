            ...
            for center in new_center_points:
                if center not in center_points:
                    if abs(center[1] - counting_line_y) <= offset:
                        ...
                        # After detecting objects and getting center points:
                        objects = tracker.update(new_center_points)
                        for objectID, centroid in objects.items():
                            #cv2.putText(frame, f'ID {objectID}', (centroid[0] - 10, centroid[1] - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255), 2)
                            if last_count != objectID and objectID > last_count:
                                vehicle_count = objectID
                                last_count = objectID
                                ...
                                csv_writer.writerow([timestamp, objectID, label, int(area), center[0], center[1], image_filename])

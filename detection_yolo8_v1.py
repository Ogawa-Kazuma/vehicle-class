...
# >>> Robust counting additions
LINE_BAND_PX = 4  # tolerance band around counting_line_y
track_info = {}   # objectID -> {"last_centroid": (x,y) or None, "counted": False}

def crossed_counting_line(prev_y, curr_y, line_y, band=0):
    """Return True if segment prev_y->curr_y crosses y=line_y (with tolerance band)."""
    if prev_y is None or curr_y is None:
        return False
    low, high = line_y - band, line_y + band
    # Opposite sides (strict crossing)
...
def save_snapshot(base_frame, attrs, save_dir, filename_root):
    ...
    img_path = os.path.join(save_dir, f"{filename_root}_snapshot_f{attrs['frame_index']}_{ts}.jpg")
...
while running:
    ...
    if start_processing and polygon_defined:
        ...
        # ---- Associate tracks to detections by nearest center (fixes mismatches) ----
        ...
        if detections:
            ...
            for objectID, trk_centroid in objects.items():
                ...
                for i in list(unused):
                    ...
                    d = (trk_centroid[0] - dcx)**2 + (trk_centroid[1] - dcy)**2  # squared distance
                    if d < best_d:
                        best_d, best_i = d, i
        ...
        # ---- Iterate tracks with matched detection for robust counting ----
        for objectID, trk_centroid in objects.items():
            # init per-track state
            if objectID not in track_info:
                track_info[objectID] = {"last_centroid": None, "counted": False}

            det_idx = track_to_det.get(objectID, None)

            if det_idx is None:
                # No detection matched (dropout) — still update trail & last centroid
            ...
            # ---- Robust line crossing via centroid segment ----
            if (not already_counted) and crossed_counting_line(
                prev_y=prev_centroid[1] if prev_centroid else None,
                curr_y=trk_centroid[1],
                line_y=counting_line_y,
                band=LINE_BAND_PX
            ):
                track_info[objectID]["counted"] = True
                counted_ids.add(objectID)  # keep original guard too
                ...
                # Save crop at the moment of counting

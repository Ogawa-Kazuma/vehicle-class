...
def open_log_files(video_path):
    ...
    summary_log_writer.writerow([video_name]) 
...
def publish_15min_report(video_path, current_dt, class_counts):
...
def process_video(video_path):
    ...
    while True: 
        ...
        if start_dt and fps > 0:
            time_elapsed_seconds = (cap.get(cv2.CAP_PROP_POS_FRAMES) - 1) / fps 
        ...
        if start_processing: 
            ...
            if start_dt and last_publish_time and current_dt >= (last_publish_time + timedelta(minutes=PUBLISH_INTERVAL_MINUTES)):
                ...
                publish_15min_report(video_path, current_dt, minute_class_counts_15min)
